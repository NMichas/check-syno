...
public class NetworkInfo {
  ...
  public static List<MibResult> get(Snmp snmp, CommunityTarget communityTarget) throws
    ...
    for (int i = 1; i < 17; i++) {
      final String nicName = OIDGetter.getInstance().getSingleOID(snmp, communityTarget, ROOT_OID + ".1." + i);
      if (nicName.startsWith("Error") || nicName.startsWith("noSuchInstance")) {
        break;
      }

      String oReceived = OIDGetter.getInstance().getSingleOID(snmp, communityTarget, ROOT_OID + ".6." + i);
      String oTransmitted = OIDGetter.getInstance().getSingleOID(snmp, communityTarget, ROOT_OID + ".10." + i);
